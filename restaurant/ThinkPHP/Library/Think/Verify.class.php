...
class Verify {
    protected $config =	array(
        'seKey'     =>  'liuc',   // 验证码加密密钥
        'codeSet'   =>  '2345678abcdefhijkmnpqrstuvwxyzABCDEFGHJKLMNPQRTUVWXYZ',             // 验证码字符集合
    ...
    public function entry($id = '') {
        ...
        // 绘验证码
        $code = array(); // 验证码
        $d = ($this->imageW - $this->length * $this->fontSize)/($this->length + 1);//验证码字符间隔
        $codeNY = ($this->imageH)/2 - ($this->fontSize)/2;	//纵坐标
        if($this->useZh){ // 中文验证码
            for ($i = 0; $i<$this->length; $i++) {
                $code[$i] = iconv_substr($this->zhSet,floor(mt_rand(0,mb_strlen($this->zhSet,'utf-8')-1)),1,'utf-8');
                imagettftext($this->_image, $this->fontSize, mt_rand(-30, 30), $this->fontSize*$i + ($i+1)*$d, $codeNY + mt_rand(10, 20), $this->_color, $this->fontttf, $code[$i]);
            }
        }else{
            for ($i = 0; $i<$this->length; $i++) {
    ...
    private function _writeNoise() {
        ...
        for($i = 0; $i < 10; $i++){
            ...
            for($j = 0; $j < 5; $j++) {
                // 绘杂点
                imagestring($this->_image, 4, mt_rand(-10, $this->imageW),  mt_rand(-10, $this->imageH), $codeSet[mt_rand(0, 29)], $noiseColor);
            }
        }
    }
